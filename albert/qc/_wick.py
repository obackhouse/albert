...
try:
    ...
    from qwick.operator import BOperator, FOperator, Projector, Sigma, Tensor, TensorSym
    ...
    from wick.operator import Projector, BOperator, Sigma, Tensor, FOperator, TensorSym
...
def EPS_dexit1(name, bspaces, ospaces, vspaces, index_key=None):
    """
    Return the tensor representation of a coupled Fermion excitation
    operator and Boson deexcitation operator.
    """
    terms = []
    for bs in bspaces:
        for os in ospaces:
            for vs in vspaces:
                ...
                operators = [BOperator(x, False), FOperator(a, True), FOperator(i, False)]
                e1 = Term(1, sums, tensors, operators, [], index_key=index_key)
                terms.append(e1)
    return Expression(terms)
...
def PB1(bspace, index_key=None):
    ...
    exp = Expression([Term(1, [Sigma(x)], [], operators, [], index_key=index_key)])
    return exp


def PB2(bspace, index_key=None):
    """
    Return the projector onto a space of double boson occupations.
    """
    x = Idx(0, bspace, fermion=False)
    ...
    exp = Expression([Term(1, [Sigma(x), Sigma(y)], [], operators, [], index_key=index_key)])
    return exp


def PE2(ospace, vspace, index_key=None):
    """
    Return the projector onto a space of double fermion excitations.
    """
    i = Idx(0, ospace)
    a = Idx(0, vspace)
    P = Projector()
